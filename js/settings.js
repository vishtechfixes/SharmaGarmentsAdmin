...
const SHOP_ID   = 'sharma-garments';   // shared/constants.js se match karo
...
function fillInputs(sh, s) {
  ...
  sv('o-reward',s.defaultVisitReward    || 'Rs.200 OFF Next Purchase');
...
window.saveAll = async function () {
  ...
  try {
    ...
    const settingsData = {
      ...
      defaultVisitReward:    gv('o-reward').trim()    || 'Rs.200 OFF Next Purchase',
...
window.delBills = function () {
  ...
  localStorage.removeItem('sg_bills');
...
window.delAll = function () {
  ...
  ['sg_users','sg_bills','sg_menu','sg_settings',
   'sg_shop','sg_feedback','sg_pts_history'].forEach(k =>
